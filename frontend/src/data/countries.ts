// Single source of truth for the country / dialling-code lists used by the sign-up and profile forms.
// Each entry is a country name, its ISO-3166 alpha-2 code, and its international dialling code. Kept as
// plain data (no runtime deps) so both the student and admin bundles can import it cheaply.
...
// Sorted by name. Dial codes are the primary country code (shared codes like +1/+7 repeat by design).
...
// Distinct dialling codes for the phone country-code selector, ordered by numeric code. Because several
// countries share a code (+1, +7, +44…), the phone prefix list is de-duplicated to one option per code.
export const DIAL_CODES: string[] = Array.from(new Set(COUNTRIES.map((c) => c.dial)))
  .sort((a, b) => Number(a.slice(1)) - Number(b.slice(1)))

/** Dialling code for a country name, or '' if unknown. */
