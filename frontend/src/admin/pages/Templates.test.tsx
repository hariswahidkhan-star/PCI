...
vi.mock('../hooks', () => ({
  useAdminQuery: (path: string | null) =>
    ({ data: path === '/api/admin/templates' ? h.resp : null, loading: false, error: null, refetch: vi.fn() }),
}))
...
describe('Admin Free Templates', () => {
  beforeEach(() => { h.resp = { rows: [], total: 0, published: 0 }; api.post.mockReset(); api.patch.mockReset() })
