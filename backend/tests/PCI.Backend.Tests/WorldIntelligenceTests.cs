...
public class WorldIntelligenceTests
{
    ...
    public void Coverage_report_counts_honestly_and_raises_the_runway_alert()
    {
        ...
        Assert.Equal(283, (int)G("mapped"));
        Assert.Equal(82, (int)G("planned"));
        Assert.Equal(283, (int)G("backed_by_published_challenge"));
        // January–September are fully authored: 273 consecutive backed days from day 1, with
        // October day 274 planned. The 60-day alert stays cleared — with real content.
        Assert.Equal(273, (int)G("runway_days"));
