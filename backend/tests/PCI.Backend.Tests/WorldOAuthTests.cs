...
public class WorldOAuthTests
{
    ...
    [Fact]
    public void Consent_is_server_enforced_for_every_client_that_is_not_first_party()
    {
        var db = NewWorldDb();
        var wid = NewWorldUser(db, "oauth-consent@x.test");
        Assert.Null(WorldOAuth.UpsertClient(db, "partner-app", "Partner LMS", "https://partner.example/cb", create: true));
        var ch = WorldOAuth.S256(Verifier);

        // Without consent: refused before anything is issued — a UI that skips the consent screen
        // cannot skip the contract.
        Assert.Equal("consent_required", WorldOAuth.IssueCode(db, wid, "partner-app", "https://partner.example/cb", ch, "S256").Err);
        Assert.Equal(0L, db.Scalar<long>("SELECT COUNT(*) FROM pciworld_oauth_codes WHERE client_id='partner-app'"));

        // With explicit consent the flow completes end to end.
        var (ierr, code) = WorldOAuth.IssueCode(db, wid, "partner-app", "https://partner.example/cb", ch, "S256", consent: true);
        Assert.Null(ierr);
        Assert.Null(WorldOAuth.ExchangeCode(db, code, Verifier, "partner-app", "https://partner.example/cb").Err);

        // The seeded first-party client never demands the ceremony.
        Assert.Null(WorldOAuth.IssueCode(db, wid, Client, Redirect, WorldOAuth.S256(Verifier), "S256").Err);

        // The flag is owner-endpoint data: flipping it changes behaviour immediately, both ways,
        // and omitting it on an unrelated update leaves it untouched.
        Assert.Null(WorldOAuth.UpsertClient(db, "partner-app", "Partner LMS", "https://partner.example/cb", create: false, firstParty: true));
        Assert.Null(WorldOAuth.IssueCode(db, wid, "partner-app", "https://partner.example/cb", ch, "S256").Err);
        Assert.Null(WorldOAuth.UpsertClient(db, "partner-app", "Partner LMS (renamed)", "https://partner.example/cb", create: false));
        Assert.Equal(1L, db.Scalar<long>("SELECT first_party FROM pciworld_oauth_clients WHERE client_id='partner-app'"));
        Assert.Null(WorldOAuth.UpsertClient(db, "partner-app", "Partner LMS", "https://partner.example/cb", create: false, firstParty: false));
        Assert.Equal("consent_required", WorldOAuth.IssueCode(db, wid, "partner-app", "https://partner.example/cb", ch, "S256").Err);
    }
