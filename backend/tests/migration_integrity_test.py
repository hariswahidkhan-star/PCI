...
def main():
    ...
    if PROVIDER == "mysql":
        ...
        # 5e. GATING (Phase 1): platform money columns are DECIMAL(12,2) on MySQL — not DOUBLE/FLOAT.
        # Generator emits DECIMAL; Migrate.EnsureMoneyDecimals upgrades legacy inexact columns in place.
        # Percentages (commission_pct, *_percentage, pass_mark_pct) are intentionally excluded.
        MONEY_NAME_SUFFIXES = ("_amount", "_price", "_fee", "_paid", "_refunded")
        MONEY_EXACT_NAMES = {
            "amount", "price", "value", "discount_value",
            "min_transaction", "max_discount", "min_payable", "renewal_fee", "exam_price",
            "application_fee", "standard_price",
        }
        inexact_money = [f"{t}.{c} is {dt.upper()}"
                         for t, cols in sorted(types.items())
                         if t not in FINANCE_TABLES
                         for c, (dt, scale) in sorted(cols.items())
                         if dt in INEXACT and (
                             c.endswith(MONEY_NAME_SUFFIXES) or c in MONEY_EXACT_NAMES)]
        chk("5e platform money columns are not stored as inexact floating point",
            not inexact_money, "; ".join(inexact_money[:12]))

        # 5f. GATING: the same money columns that exist must be DECIMAL with scale 2 (cents).
        wrong_scale = [f"{t}.{c} is {dt.upper()} scale={scale}"
                       for t, cols in sorted(types.items())
                       if t not in FINANCE_TABLES
                       for c, (dt, scale) in sorted(cols.items())
                       if (c.endswith(MONEY_NAME_SUFFIXES) or c in MONEY_EXACT_NAMES)
                       and dt == "decimal" and scale != 2]
        chk("5f platform money DECIMAL columns use scale 2",
            not wrong_scale, "; ".join(wrong_scale[:12]))
        ...
        static_bad = []
        for col in sorted(MONEY_EXACT_NAMES | {
            "standard_amount", "default_discount_amount", "discount_code_amount", "final_amount",
            "amount_paid", "amount_before", "discount_amount", "waived_amount", "original_amount",
            "payable_amount", "amount_refunded",
        }):
            if re.search(rf"\b{re.escape(col)}\s+DOUBLE\b", mysql_schema, re.I):
                static_bad.append(col)
        chk("5g schema.mysql.sql money columns are DECIMAL, not DOUBLE",
