...
# schema.mysql.sql is GENERATED by tools/sqlite_to_mysql.py but hand-tuned afterwards: money columns are
# widened to DECIMAL(12,2) and long keyed columns are given index prefix lengths. Regenerating the file
# silently reverts every money column to DOUBLE, and sections 1-4 compare NAMES only, so they cannot see
# it. That is the gap this section closes.
def mysql_column_types():
    """{table: {column: (data_type, numeric_scale)}} — the live MySQL types, lower-cased."""
    import pymysql
    con = pymysql.connect(database=MIG_DB, **MYSQL)
    cur = con.cursor()
    cur.execute("SELECT LOWER(table_name), LOWER(column_name), LOWER(data_type), numeric_scale"
                " FROM information_schema.columns WHERE table_schema=%s", (MIG_DB,))
    out = {}
    for t, c, d, scale in cur.fetchall(): out.setdefault(t, {})[c] = (d, scale)
    con.close()
    return out
...
def main():
    ...
    if PROVIDER == "mysql":
        ...
        inexact_finance = [f"{t}.{c} is {dt.upper()}"
                           ...
                           for c, (dt, _s) in sorted(types[t].items())
                           if dt in INEXACT]
        ...
        wrong_minor = [f"{t}.{c} is {dt.upper()}"
                       ...
                       for c, (dt, _s) in sorted(cols.items())
        ...
        # 5e. NON-GATING INVENTORY of the platform-wide condition recorded as P0-Q.
        #
        # schema.mysql.sql is generated by tools/sqlite_to_mysql.py and then hand-tuned to widen money
        # columns to DECIMAL(12,2). Regenerating it reverts them to DOUBLE, and sections 1-4 compare NAMES
        # only, so nothing in CI could see that happen. This block makes it visible on every run.
        #
        # It is deliberately NOT gating: the columns below are pre-existing platform history, and changing
        # their types is a migration against live tables that belongs to its own reviewed change, not to a
        # test tightening. Failing here would block every unrelated commit until that landed. What the
        # inventory guarantees instead is that the number cannot grow unnoticed.
        legacy_inexact = [f"{t}.{c}"
                          for t, cols in sorted(types.items())
                          if t not in FINANCE_TABLES
                          for c, (dt, _s) in sorted(cols.items())
                          if dt in INEXACT and (c.endswith("_amount") or c.endswith("_price") or c in (
                              "amount", "price", "value", "discount_value", "commission_pct",
                              "min_transaction", "max_discount", "min_payable"))]
        print(f"  NOTE  P0-Q inventory: {len(legacy_inexact)} pre-existing money column(s) are DOUBLE/FLOAT")
        for col in legacy_inexact[:20]:
            print(f"          · {col}")
        if len(legacy_inexact) > 20:
            print(f"          … and {len(legacy_inexact) - 20} more")
