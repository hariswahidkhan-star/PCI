...
public static class Chat
{
        ...
        Func<HttpRequest, AdminCtx?> adminFromReq, Func<HttpRequest, string, Func<AdminCtx, IResult>, IResult> gate)
    {
        ...
        app.MapPost("/api/chat/start", async (HttpRequest req) =>
        {
            ...
            var greeting = (name.Length > 0 ? $"Hello {name}! " : "Hello! ")
                + "I'm the PCI Assistant. Ask me about the PCI certifications (PCL-AI, PFL-AI, PDL-AI), eligibility, fees, enrolment or membership — or ask to talk to a person at any time.";
            long id = 0;
            db.Transaction(() =>
            {
    ...
    const string SMALLTALK_MENU = " I can help with the PCI certifications (PCL-AI, PFL-AI, PDL-AI), the Body of Knowledge, eligibility, fees, enrolment, the exam or membership — what would you like to know? You can also ask to talk to a person any time.";
