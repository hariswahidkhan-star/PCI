        ...
        app.MapPost("/api/honorary-application", async (HttpRequest req) =>
        {
            ...
            if (Notify.Enabled(db, "honorary"))
            {
                ...
                Notify.Email(db, null, email, ackSubject,
                    ...
                    $"<p>Honorary recognition is conferred at the board's discretion. It involves no examination and is separate from the examined PCP-AI credential. We will email you once a decision has been made.</p>" +
            ...
            if (status.Length > 0 && status != "pending_review" && Notify.Enabled(db, "honorary"))
            {
                ...
                var (subject, body) = status switch
                {
                    "approved" => ("Congratulations — Honorary Fellow (PCI) conferred",
                        ...
                        $"<p>You can verify it any time at <a href=\"{baseUrl}/verify.html?id={WebUtility.HtmlEncode(awardNo)}\">{baseUrl}/verify.html?id={WebUtility.HtmlEncode(awardNo)}</a>. This is an honorary recognition, distinct from the examined PCP-AI credential.</p><p>— Project Controls Institute Global</p>"),
