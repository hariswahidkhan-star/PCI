            ...
            for (int i = 0; i < 20 && reference is null; i++)
            {
                ...
                try
                {
                    appId = db.ExecuteReturningId(@"INSERT INTO honorary_applications
                        (reference,first_name,last_name,email,mobile,country,city,nationality,job_title,employer,years_experience,industry,highest_qualification,professional_certifications,relevant_experience,professional_summary,certification_id,declaration,status)
