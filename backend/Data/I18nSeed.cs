...
{
    const int Version = 5;   // v5: brand casing "PCI ai" → "PCI AI" (rekeyed wordmark/congress regions again)
    ...
    public static void Apply(Db db)
    {
        try
        {
            ...
            using (var doc = JsonDocument.Parse(File.ReadAllText(path)))   // scoped: the ~11 MB DOM is
            {                                                              // freed before any DB work
            ...
            // Commit in bounded transactions (~6k rows each) instead of one giant one: on a small
            // container (Render Starter, 512 MB) a single ~90k-row transaction accumulates enough
            // dirty-page state to matter at boot. INSERT OR IGNORE keeps re-runs idempotent, and the
            // version stamp is written LAST, in its own transaction — a crash mid-seed simply
            // resumes on the next boot.
            const int stmtsPerTxn = 20;
            for (int start = 0; start < rows.Count; start += chunk * stmtsPerTxn)
            {
                var upper = Math.Min(rows.Count, start + chunk * stmtsPerTxn);
                db.Transaction(() =>
                {
                    for (int i = start; i < upper; i += chunk)
                    {
                        var take = Math.Min(chunk, upper - i);
