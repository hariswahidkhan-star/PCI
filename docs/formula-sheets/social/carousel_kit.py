...
/* Running co-brand bar. Both marks ride every slide: a deck about a relationship that
   shows one party's mark on six of seven pages is a press release about that party. */
...
# subject is a relationship; left None, the bar carries our mark alone.
...
def lockup(n: str) -> str:
    ...
    return ('<div class="top">'
            '<img class="tile" src="/backend/wwwroot/assets/logo.svg" alt="">'
...
def render_deck(slides: dict, work: pathlib.Path, out: pathlib.Path,
    ...
    for name, html in slides.items():
        ...
        # the deck's own length fills the counter, so adding a slide needs no edits
        src.write_text(html.replace("__TOTAL__", f"{len(slides):02d}"), encoding="utf-8")
