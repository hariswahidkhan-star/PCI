...
The repository now has deeper deterministic coverage for the public visitor, candidate, PCI operator, honorary/founding applicant and institution-partner personas. The browser tranche now contains **22 spec files, 58 Chromium tests and 82 configured executions** across Chromium, Firefox, WebKit, Pixel 7 and iPhone 15 profiles. It creates story-named success-state screenshots in the Playwright report, in addition to automatic failure screenshots, traces and video.
...
- the current PR #73 head `b92e632` has been merged into this workspace; GitHub exposed no commit-status or pull-request workflow run for that SHA at the time of inspection, so this report does not claim it is green;
- the deeper local change set passes every runnable local gate (six backend logic suites, Python syntax, ESLint with zero errors, TypeScript, **79 Vitest tests**, production builds, npm production audit with zero vulnerabilities, and Playwright discovery of all **82** executions);
...
| A3 | Anonymous visitor | PENDING-CI | Y | `A3-arabic-rtl.png` plus per-language captures | Full en→ko→ar→es→fr→zh→ru copy/navigation sequence, RTL, persistence and switcher are browser-driven. |
| A4 | Anonymous visitor | PENDING-CI | Y | `A4-announcement-visible.png` | Visible dialog, accessibility, dismissal persistence and active-language announcement copy are asserted. |
| A5 | Honorary applicant | PENDING-CI | Y | A5/G9/E5 captures | Application → shortlist → one-time IDV → board approval → account setup → award download/verify → revocation is browser-driven. |
...
| A10 | Anonymous visitor | PENDING-CI | Y | One attachment per named policy | Every named policy route is opened, checked for crawlable content and visually evidenced. |
...
| B4 | Candidate | PENDING-CI | Y | `B4-password-reset-requested.png` | Browser request is non-enumerating and the real delivery record is asserted; token expiry/single-use remains API-covered. |
| B5 | Admin-provisioned candidate | PENDING-CI | Y | `B5-setup-password-complete.png` | Admin setup link → set password → one-time token rejection → portal login is browser-driven. |
...
| B7 | Candidate | PENDING-CI | Y | `B7-onboarding-complete.png` | Every onboarding step is completed and the profile reaches 100%. |
...
| C4 | Candidate/admin | PENDING-CI | Y | `C4-certification-scoped-code-preview.png` | Admin creates a PML-AI-scoped code; Billing rejects it for PCL-AI, previews it for PML-AI and shows savings/total. |
| C5 | Founding applicant/admin | PENDING-CI | Y | C5/G10 captures | Code creation, evidence application, board approval, membership and exam entitlement are browser/API asserted. |
| C6 | Candidate | PENDING-CI | Y | `C6-printable-payment-receipt.png` | Manual settlement appears in Billing and invoices; the printable receipt is opened and verified in a browser page. |
...
| D7 | Candidate/proctor admin | PENDING-CI | Y | D7/G7 live-board captures | Candidate heartbeat/critical event/chat → proctor live board/reply → candidate receipt → review is browser-driven. |
...
| D9 | Support admin/candidate | PENDING-CI | Y | `D9-support-view-banner.png` | Admin opens support view, the permanent banner renders, mutation remains 403, session ends and history/audit are checked. |
...
| E4 | Candidate/admin | PENDING-CI | Y | `E4-document-acknowledged.png` | Admin assignment → student acknowledgement/download → audit is browser-driven. |
| E5 | Honorary member/admin | PENDING-CI | Y | A5/E5/G9 captures | Member account setup, honorary PDF download, public verification and revocation are driven end to end. |
| E6 | Candidate/admin | PENDING-CI | Y | E6/G6 credential captures | Admin links a credential to a real student, uploads custom PDF, holder downloads, public verifies, then revoke/reinstate is asserted. |
...
| F2 | Candidate/admin | PENDING-CI | Y | `F2-cpd-approved.png` | Student submission → new CPD review console → approval → updated member total is browser-driven. |
...
| F5 | Candidate | PENDING-CI | Y | F5 TOTP and session captures | Student TOTP enrolment, step-up login, recovery-code disable and other-session revocation are browser-driven. |
| G1 | Owner/admin | PENDING-CI | Y | G1 TOTP/recovery captures | Admin enrols TOTP, proves step-up, recovers, and cannot silently replace an active factor. |
...
| G3 | Members admin | PENDING-CI | Y | G3/D9 consolidated console captures | Real-student search, drawer, journey/security state, support view and audit attribution are driven together. |
| G4 | Finance admin | PENDING-CI | Y | G4 reconciliation captures | Manual payment, full waiver, reconciliation, reverse and membership lapse are asserted in the admin UI/API. |
...
| G6 | Credential admin | PENDING-CI | Y | G6/E6 captures | Linked issuance, custom upload, revoke/reinstate and holder/public reflections are one browser journey. |
| G7 | Exam/proctor admin | PENDING-CI | Y | G7/D7 captures | Live proctor board, evidence stream, bidirectional chat and review are driven with a real candidate. |
...
| G9 | Honorary board admin | PENDING-CI | Y | A5/G9 captures | Shortlist, IDV link, submitted identity evidence, approval/setup-link delivery and revocation are browser-driven. |
| G10 | Codes admin | PENDING-CI | Y | C4/C5/G10 captures | Scoped discount and manual founding-application codes are created in the real console and exercised by a student. |
...
| G17 | Auditor | PARTIAL | Y | Cross-story audit assertions | Acting-admin attribution is corrected and asserted for the newly covered privileged journeys; legacy student/admin ID collision still needs an actor-type schema migration. |
| G18 | Owner/access admin | PENDING-CI | Y | `G18-viewer-least-privilege.png` | Owner creates a scoped viewer; nav visibility and forbidden API sections are asserted. |
| G19 | Owner/settings admin | PENDING-CI | Y | `G19-settings-round-trip.png` | Owner changes a student-panel setting and verifies persistence plus acting-admin audit attribution. |
...
| H3 | Institution user | PENDING-CI | Y | `H3-private-document-download.png` | Partner document listing/download and cross-institution isolation are browser/API asserted. |
...
7. **Account security:** student/admin reset, setup-link and TOTP journeys are browser-covered; an active admin factor can no longer be overwritten by restarting setup, and recovery codes are consumed.
8. **Privileged audit attribution:** finance, student, credential, proctor-support, founding, honorary, directory and event mutations now record the acting admin and retain the subject in details.
9. **Operator workflows:** the admin console gains a CPD review screen; credential issuance can be linked to a real student/certification; Honorary approval surfaces a fallback setup link; Billing previews scoped codes for the selected product/certification.
10. **Honorary lifecycle:** shortlisting no longer clears the newly generated IDV link, and approval returns the one-time setup URL to an owner without relying solely on outbound email.
11. **CSV security after upstream merge:** all export paths use the shared encoder; formula markers are neutralised even after leading whitespace while genuine signed numbers remain numeric.
...
2. **P1 — execute the 82 browser cases:** the journey implementations and discovery are complete, but their runtime status remains pending until CI provides the .NET host, database and browser engines.
3. **P1 — finish audit modelling:** add an explicit actor type/actor ID to `audit_logs`; the current single numeric `user_id` can collide between student and admin sequences even though new privileged writes attribute the acting admin correctly.
...
5. **P2 — remaining UI breadth:** certification/content/SEO/question-bank mutation-to-public journeys and every communications template still need consolidated browser evidence.
