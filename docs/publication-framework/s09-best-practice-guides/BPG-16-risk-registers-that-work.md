...
Almost every project has a risk register. A small minority have one that changes a decision in any given
month. The gap is not effort — unused registers are often meticulously maintained — and it is not tooling.
It is that the register was built to demonstrate that risk management is happening rather than to make risk
decisions, and an artefact built for demonstration acquires a predictable shape: many entries, vague
statements, team-level ownership, scores that never move, actions without dates, and a review that reads the
top ten aloud and closes.

The diagnosis matters because the remedies differ. A register failing from neglect needs attention. A
register failing from purpose needs rebuilding around the decisions it is meant to inform: what we fund,
what we mitigate, what we escalate, what we tell the client.

## 2. Cause, event, effect — and why the structure is not pedantry

A risk entry that can be managed has three separable parts.

**The cause** is a condition or dependency that exists now. It is not uncertain; it is a fact about the
project. "The permanent power connection is delivered by a utility outside the contract."

**The event** is the uncertain thing. It either happens or it does not, and a probability attaches to it.
"Energisation is granted later than the commissioning window requires."
...
mitigation attacks — you cannot reduce an event's probability except by acting on its cause, which is why
registers full of causeless risks generate mitigations that are really just monitoring. The **event**
carries the probability and can be declared closed when the uncertainty resolves. The **effect** carries the
impact, feeds quantification, and determines who should own the entry, because a risk belongs to whoever
owns the objective it threatens, not to whoever first mentioned it.

Two tests. If the cause is not a present fact it is a second risk, and the two should be separated. If the
event cannot be declared to have happened or not happened on a given date it is a condition, and it belongs
in the assumptions or the issue log.
...
aggregating them can only be assigned to the project manager, and the mitigation can only be "manage the
project". That is a job description, not a control.

**It can never be closed.** The project is at risk of delay from first day to last, so the entry sits
permanently with its score unchanged — teaching every reader that entries on this register do not move. One
immortal entry is enough to establish the culture.

**It double-counts.** If the register also carries the specific events that would cause delay, the aggregate
entry counts them again. Any quantification built on it is then wrong by an unknown amount, in the direction
of overstating exposure — which sounds prudent until a contingency request must be defended line by line.
...
fails an inspection, under-resources a work front — each with its own cause and response. And something that
has already happened is not a risk: it is an issue, and it belongs in the issue log with an action and a
date. A register carrying issues has stopped distinguishing what might happen from what has, which is the
one distinction it exists to make.

## 4. Qualitative scoring: what it orders, and what it cannot

Almost every register scores probability and impact on ordinal bands — typically one to five — and
multiplies them into a matrix score. Used for what it is good at this is sound and fast: it triages a large
register quickly, needs no cost model, works in a workshop with people who will not tolerate a quantitative
exercise, and identifies which entries deserve quantification.
...
**Use the matrix to triage, not to rank for funding.** It answers "which entries deserve more analysis", not
"which entries drive the contingency". Any register whose top ten is selected by matrix score has the wrong
ten in it.

**Define the bands in the units of the project, and write them on the register.** "Low, medium, high" is
re-interpreted by everyone who uses it; a stated money range and a stated probability range are at least
argued about consistently. Calibrate so that band 5 is genuinely material — a band-5 threshold set at a
figure the project spends monthly makes the whole top row meaningless.

**Score impact against a stated objective.** A risk can be band 2 for cost and band 5 for schedule. One
combined score hides which objective is threatened, and therefore who should own it.

Where a project has an appetite statement, the matrix is also where it is expressed — by drawing the
escalation line on the grid rather than leaving each chair to apply their own tolerance. That is professional
judgement made explicit, which is the only form in which it can be reviewed.
...
**Every response has an action, an owner and a date in the future.** An action without a date is an
intention. A register where a third of the action dates have passed has told you how much attention it
receives, more reliably than any maturity assessment.

**Responses are typed, and the type is honest.** The threat strategies are avoid, transfer, reduce and
accept; the opportunity strategies are exploit, share, enhance and accept. Most entries claiming to be
mitigated are in fact accepted with monitoring, and calling that "mitigate" inflates the project's apparent
control. Conscious acceptance with contingency held against it is legitimate and often correct; disguising
acceptance as mitigation is not.

**A response is justified when it reduces probability times impact by more than it costs.** That test is
arithmetic and should be shown: the register carries the pre-response and post-response assessment and the
response's cost, so the decision is visible. Responses whose cost exceeds the exposure they remove are
recorded as rejected with a reason, which is what stops the same proposal returning every quarter.

**Every entry carries a review date.** A risk whose score has not moved in three cycles is either genuinely
stable — in which case say so and review it less often — or unexamined. Recording which takes one column.
...
weather season behind several delays, one overheated supply market behind several vendor slips. They make
coincidence far more likely than independent arithmetic suggests, raising the funded confidence level while
leaving the expected-value sum exactly where it was.
...
chosen and governed; `BPG-17 — Quantitative schedule risk analysis` owns what happens when the register is
mapped onto the schedule and simulated.

One connecting discipline belongs to the register rather than the model: **risk loading**. Each entry that
will be quantified names the specific activities and cost lines it would strike. Without that mapping the
model runs on a parallel set of assumptions and the register merely resembles it; with it, the register is
the model's input and a change to one is visible in the other.

## 7. How this goes wrong

**The register is a deliverable, not a tool.** Produced because the framework requires one, reviewed because
the review is in the schedule, consulted by nobody in between. The tell is that nothing in the project plan
changes as a result. A register that has never caused a schedule change, a contingency draw or a contract
negotiation is not managing risk.

**Entries are effects, so nothing can be closed.** Covered in §3. The symptom is a register whose population
only ever grows.
...
contingency and, occasionally, the ones that end the project.

**Mitigations are monitoring in disguise.** "Monitor closely", "maintain dialogue with the supplier",
"escalate if required". None changes probability or impact, so the residual assessment should be unchanged —
yet the register almost always shows a reduced residual score once they are added.

**Everything is owned by the project manager.** A register with one owner is a to-do list belonging to
someone who already has one.

**Impact is scored on the project, not on the objective.** "High" means high for schedule in one entry and
high for reputation in another, and the two are then compared as though commensurable.

**Opportunities are absent.** A register with no upside entries is evidence about the workshop, not the
project, and it biases the contingency conversation, because what would net against threats is not on the
page.

**The register outlives its assumptions.** Scores set at sanction are still there at handover. The risk
profile a project starts with is not the one it carries, so re-identification at each gate is not optional.

**Quantification double-counts responses.** The entry is quantified at its pre-response impact and the
response's cost is also in the budget. The exposure is funded twice and the request cannot survive scrutiny.
...
**Where the tail lives.** R-31 can cost 1,200,000 in a single event — 3.75 times the register's entire
expected-value sum (`1,200,000 ÷ 320,000 = 3.75`) — and the matrix places it below the escalation line. If
the top ten were selected by matrix score, the project's largest single loss would not be in it.

### 8.5 Why this happens, in one line

The impact bands step by roughly an order of magnitude while the band numbers step by one. Multiplying band
numbers multiplies rank positions; multiplying probability by impact multiplies a proportion by an amount of
money. Only the second produces a quantity you can add up, compare or fund.

### 8.6 What follows for this register

Use the matrix to decide which entries go forward for quantification, with the forward criterion set on
*either* dimension so a band-4 or band-5 impact goes forward regardless of probability. Order the funding
conversation by expected value, and hold the heavy-tail entries separately because their expected value
understates what one occurrence does. Then ask what R-24 and R-31 have in common: a shared driver — one
contaminated-land condition that also delays the vendor's foundation works, say — is the pairing that moves
the funded level.

The assumptions travel with the numbers: probabilities are workshop point estimates with no range around
them; impacts are single figures, not distributions; nothing here nets opportunities against threats; and the
expected-value sum is a starting point for contingency, not a contingency figure. `BPG-10` takes it on.
...
half the entries have not moved in three cycles, more than a third of actions are overdue and nothing closed
all cycle is not being used; it is being maintained. And 84 is itself a finding — a project board cannot
manage 84 of anything, so either the register needs a working subset with a stated selection rule, or most
entries should be closed, merged or moved to a watch list on a longer cycle.

Run these counts before the review meeting rather than after it. They take minutes, they are not disputable,
and they change what the meeting is about.
...
A register that passes this list is smaller than the one it replaced and is read by people who are not paid
to read it. That is the only durable evidence that risk management is happening: the register changed what
the project did, and someone can name the month it happened.
...
ISO 31000 is the international risk-management standard most often referenced in this area. This guide
reproduces none of its text, structure or clause numbering; where its principles are relevant — that risk
management is integrated into decisions rather than run alongside them, and that it addresses uncertainty in
both directions — they are explained in the Institute's own words. No survey data, benchmark or industry
average is cited, because none was verified for this guide.
