...
|---|---|---|
| **In the loop** | The system proposes; a human evaluates and decides; nothing proceeds without the decision | Any output meeting a test in §3 |
| **On the loop** | The system acts; a human monitors, samples and can intervene | High-volume, low-unit-consequence work with a measured error rate and an active monitoring duty (Tier 3, `AIG-08` §4) |
| **Out of the loop** | The system acts; no human reviews before or after | Only where the action is trivially reversible, has no external reliance and no contractual effect — in a controls function, almost nothing |
...
served, a payment certified — these cannot be quietly undone next period. Reversibility, not size, is the
first discriminator.
...
**3. Will someone outside the project rely on it?** A board, a client, a lender, an auditor, a certifier.
External reliance converts an internal analysis into a representation, and representations are made by
people.

**4. Is the question fundamentally *why*, and is the answer contested?** Attribution of cause — who caused
the delay, whether the escalation was foreseeable, whether the rework is a defect or a variation — is the
substance of most disputes. A model can assemble the evidence; it cannot hold a position.
...
someone by a system nobody can explain is indefensible in every jurisdiction.

There is one gating condition on top of the tests: **an output that cannot be reconstructed cannot carry
any of these decisions**, whatever its apparent quality — see `AIG-09 — Bias, explainability and
auditability` §8.
...
|---|---|---|
| **Baseline change** | Assemble the change pack, quantify time and cost impact, check the change register for related items | Decide whether the change is approved, on what scope and what value, and sign the baseline revision (`BPG-04`) |
| **Contingency release** | Match the drawdown request to the risk that materialised, compute the remaining balance and burn rate | Decide the release, its amount and the authority level it requires (`BPG-10`, and §5 below) |
...
| **Acceptance of a subcontractor's programme or valuation** | Check logic, compare against contract dates, reconcile quantities | Accept, reject or accept with reservation — an acceptance is a commercial act |
...
| **Decisions about people** | Nothing that scores or ranks individuals without a specific, lawful and explainable basis | Make the decision, on evidence they can explain to the person affected |
| **Changing a rule of credit or a measurement basis** | Model the effect of the change | Decide it, and disclose the discontinuity in the reported trend |

Two things this table is not. It is not a list of tasks AI should be kept away from — every row has a
substantial AI contribution in the middle column, and refusing that contribution is its own failure. And
it is not a maturity statement that will relax as tools improve: the tests in §3 are about the nature of
the decision, not the capability of the tool. A more capable model does not make a contested entitlement
position less contested.
...
percentage of budget at completion (BAC), or a category. A second, parallel ladder guarantees divergence
and gives an approver two answers to choose between.
...
against the same risk, aggregate for the purpose of the threshold — and the rule is written down, because
the split is rarely deliberate and always available. §7's worked example is built on this point.

Add one AI-specific provision, and only one: **the approver must be able to state the basis of the
proposal in their own words.** If they cannot, the proposal is not ready for approval — a rule that is
cheap to apply and catches the rubber-stamp before it becomes habitual.

## 6. What a signature actually asserts

A signature on an AI-assisted output asserts four things. Anyone signing should know all four, because
they are what the signature will be tested against.
...
3. **The assumptions are stated** in the same place as the answer, so a reader can challenge them.
4. **The differences between the model's output and the signed output are recorded**, with reasons.

"The model produced it" is not a defence, and neither is "the tool is approved". The approved-tool register
governs which tools may be used; it says nothing about whether *this* output is right. The verification
record of `AIG-09` §3 is what the signature rests on.

The corollary is uncomfortable and worth stating plainly: **a reviewer who could not have produced the
output by another route is not in a position to sign it.** That does not mean every signatory must redo
every calculation. It means the capability must exist in the review, and where it does not, the output goes
to someone who has it. This is why the deskilling problem in `AIG-12 — The AI-literate controls
professional` is a governance problem and not a training-department problem.

## 7. How rubber-stamping starts

It does not start with negligence. It starts with a run of correct outputs.

The sequence is consistent. The tool is accurate for several cycles. Review finds nothing. Review time
falls, because nothing rewards it. The reviewer begins checking that the output looks reasonable rather
than that it is right — a much faster operation, and indistinguishable from the outside. Then an output is
wrong in a way that looks reasonable, and it passes.

Three detections, none expensive.

**Review time per output.** If the time booked to verification has fallen materially while volume has not,
that is the signal. It is measurable if verification is a costed line (`AIG-08` §10) and invisible if it
is not.
...
reconstruct it (`AIG-09` §1). This is the only test that distinguishes the two cases above, and it takes
under an hour.
...
at step two carries the error forward, and every later step builds on it plausibly. The error is not just
propagated; it is polished. By the final output, the workings that would have exposed it are gone.

The response is to move verification **from per-output to per-workflow**. The professional assures the
design of the chain — which steps are permitted, which data each step may touch, where the chain must stop
— and inserts checkpoints where a consequential intermediate result is inspected before the chain
continues. Sensible checkpoints in a controls workflow are: after data extraction and before computation
(is this the right cut?), after computation and before narrative (do the numbers reconcile?), and before
anything leaves the function.

The audit trail must now record the chain, not just the answer. And the decisions in §4 do not move: a
chain may prepare a contingency release pack end to end, and the release is still decided by the person
with the authority.
...
**Signature without capability.** The person who signs cannot recompute, and does not have access to
someone who can within the reporting timetable. The signature is a formality and everyone involved knows
it.
...
**Reasonableness substituted for correctness.** Six good cycles, then review becomes a glance. The failure
is invisible until an output is wrong and reasonable at the same time.

**Chains verified only at the end.** The final pack is checked carefully and every intermediate step is
trusted. An error introduced at extraction is confirmed by everything downstream that was built on it.

**Escalation with no route.** The framework says contested items escalate; nothing says to whom, by when,
or what happens if the reporting deadline arrives first. Under time pressure the item is signed.
...
**Step 4 — what AI contributed and what it did not.** The tool matched the drawdown to the risk, computed
the remaining balance (`2,400,000 − 345,000 = USD 2,055,000`) and the burn rate, and assembled the
evidence. It did not decide that the risk had materialised, that the amount was right, or that the
remaining contingency is adequate for the risks that have not.
...
**Interpretation.** The arithmetic is trivial and the control is not. Without the aggregation rule of §5,
three defensible individual approvals produce an undisclosed concentration; each approver acted within
authority and nobody saw the total. Note also what the example does not do: it does not use the model's
confidence in the match as a reason to lower the authority level. The threshold is a property of the
decision's consequence, and the consequence of an over-drawn contingency is unchanged by how the
recommendation was produced.

## 11. Checklist

- [ ] The framework states, per use, whether the human is in the loop or on the loop — and where it says on the loop, the error rate is measured and the monitoring duty has a named owner.
...
- [ ] Authority thresholds are expressed on the decision, not on whether AI was involved.
- [ ] Thresholds use the project's existing delegated authority schedule, not a parallel one.
- [ ] An aggregation rule exists for related decisions within a period.
- [ ] Approvers must be able to state the basis of a proposal in their own words.
...
The line is not drawn to keep AI out of the work. It is drawn so that when the decision is questioned,
there is a person who made it, on grounds they can state — which is the only thing that has ever made a
project controls number worth anything.
