...
with a weighted coverage measure. It includes a worked gap assessment and a checklist.
...
propositions they claim to be able to act on are in `PCB-04`; the limits on that claim — point-in-time
validity, no accreditation, no outcome guarantee, separation from membership grades — are in `PCB-01 — The
Project Controls Body of Knowledge — executive summary` §5, and should be read before relying on it.

Three points bear specifically on hiring. **Eligibility is three years' professional experience in any
field**, not three years in project controls, so sector and domain experience remain yours to assess. **One
route requires no examination**: of the three published routes — standard, founding member, honorary fellow
— the honorary route involves no sitting, so verify which route earned a credential that matters to a
...
rewritten every time the tool changes. A published framework lets you specify competence and treat the tool
as a training item.

The method takes about an hour. Mark each of the thirteen domains **required**, **supporting** or **not
applicable**; then for each *required* domain name the artefact the post-holder owns, its frequency, and its
reviewer. That last step turns a framework into a specification. "Domain 6 required" tells a candidate
nothing; "owns the monthly earned value report across fourteen control accounts, published to the project
board by working day five, reviewed by the controls manager" tells them exactly what they are applying for.
...
against what?" Do not ask candidates to reproduce examination content: sample items are published study
material, live content never is.
...
index.* A controls function of nine people. For each domain the head of function records how many of the
nine can work **unsupervised** in it: one deliberately blunt judgement made in a single session, not a
survey. Group A: D1 = 2, D2 = 1, D3 = 7, D4 = 6. Group B: D5 = 8, D6 = 5, D7 = 4, D8 = 6, D9 = 2, D10 = 7,
D11 = 1, D12 = 4. Group C: D13 = 1. Figures computed unrounded, shown to one decimal place.

**Step 1 — unweighted coverage.** Competent person-domains `= 16 + 37 + 1 = 54`; slots `= 13 × 9 = 117`.
`Unweighted coverage = 54 ÷ 117 = 46.2 %`

**Step 2 — coverage by group**, the mean count across the group's domains, over the nine people.
...
That sits 5.6 percentage points below the unweighted 46.2 per cent, because the thinnest coverage — one
person in nine in Domain 13 — is where a fifth of the framework's weight rests on a single domain. The
headline number flatters the function; the weighted one does not.
...
your team: the *index* is twice as sensitive to Domain 13, which is not to say the training is twice as
valuable to your projects. If the live problem is a disputed revenue position, option 2 is the right
decision and the index the wrong instrument. Use it to find where you are thin, never to choose what to do
about it.
...
**Sponsor certification deliberately.** An examination validates knowledge; it does not substitute for the
development above. Fees are route-dependent and discountable: `[CONFIRM: examination fee — platform seeds
USD 500, legacy candidate pack states USD 350]`. Budget also for recertification on a three-year cycle with
its mandatory AI-currency component; the student portal shows a target of thirty hours of continuing
professional development per cycle, but `[CONFIRM: the binding CPD requirement, to be published with the
recertification rules]`.

## 6. What not to use the framework for

**Not as a pay scale** — PCI publishes no salary data, and any figure attributed to the Institute today is
not ours. **Not to screen out non-certified candidates** — its hiring value is the structure it gives your
questions, which works identically on people who hold nothing. **Not as accreditation, to your client** —
"our team is PCI-certified" is accurate; implying accredited or government-recognised certification is not.
**Not for the sibling credentials** — it is PCL-AI only, and no blueprint exists yet for PFL-AI or PML-AI.

## 7. Checklist

- [ ] Every open post marks each of the thirteen domains required, supporting or not applicable
- [ ] Every required domain names its artefact, frequency and reviewer; no post requires all thirteen
- [ ] Group weights are not being used as a role or workload weighting
...
- [ ] Team coverage is recorded per domain, with single points of failure flagged
- [ ] Development objectives are artefacts produced and reviewed, not courses attended
- [ ] Any credential relied on has been verified, including which route earned it
