...
def figure_rows(book: str) -> list:
    ...
    out = []
    for f in sorted((ROOT / book / "manuscript").glob("domain-*.md")):
        # A caption is the bolded run that opens the blockquote and it wraps freely, so the wrapped
        # continuation lines are re-joined before matching. Matching a single line truncated every
        # caption that wrapped, and stopping at the first full stop truncated every caption holding a
        # decimal ("at 1.30×" became "at 1").
        block = []
        for line in f.read_text(encoding="utf-8").splitlines() + [""]:
            if line.startswith(">"):
                block.append(line.lstrip(">").strip())
                continue
            m = re.match(r"\*\*Fig (\d+\.\d+\.\d+) — (.+?)\*\*", " ".join(block)) if block else None
            block = []
            if m:
                fid = m.group(1)
...
def build(book: str) -> str:
    ...
    L = [f"# Appendices — {title}", "",
         "> **Derived, not maintained.** Every table in these appendices is generated by the Institute's",
         "> publication build from the shared registries and from the golden-answer suite itself, so none",
         "> of it can drift from the chapters or overstate what has been verified.", ""]
    ...
    for dn in sorted(titles):
        n = counts.get(dn, 0)
        # The cross-domain bucket is a real category, but an empty one is noise: a volume whose checks
        # all attribute to a single domain gets no row rather than a row of zeroes.
        if dn == 0 and n == 0:
            continue
        L.append(f"| {dn} | {titles[dn]} | {n:,} |")
    L += [f"| | **Total** | **{total:,}** |", "",
          ...
          "be evaluated is recorded as a failure, never passed over as a skip.", "",
    ...
    if drawn == len(figs):
        L += [f"**{len(figs)} figures**, all of them drawn as PCI-original artwork generated from source",
              "specifications maintained by the Institute. Each is also specified in words in the chapter,",
              "stating every plotted value,"]
    else:
        L += [f"**{len(figs)} figures**, of which **{drawn}** are drawn as PCI-original artwork generated from",
              "source specifications maintained by the Institute. The remaining",
              f"{len(figs) - drawn} print as their own written specifications, stating every plotted value,"]
    ...
    L += ["",
          ...
          "verification is recorded in the Institute's corpus gate report rather than glossed over.", "",
