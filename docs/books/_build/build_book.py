...
def inject_figures(book_dir: pathlib.Path, corpus: str) -> str:
    """Replace each figure specification with its rendered artwork, carrying the spec's alt text.

    The manuscripts already write an `Alt text:` sentence into every figure specification, and it was
    being discarded — every injected image reached the PDF with no alternative text at all, while the
    conformance matrix claimed alt text as a *correction* over the previous book. The spec blockquote
    spans several lines, so the alt sentence has to be gathered from the whole block rather than from
    the line the figure id sits on, which is why the earlier line-at-a-time version could not have
    found it.
    """
    lines = corpus.split("\n")
    out, injected, with_alt, i = [], 0, 0, 0
    while i < len(lines):
        m = re.match(r"> \*\*Fig (\d+\.\d+\.\d+) — ([^.*]+)", lines[i])
        if m:
            fid, title = m.group(1), m.group(2).strip().rstrip(".")
            # the specification is this line plus the following consecutive quoted lines
            j = i
            while j < len(lines) and lines[j].startswith(">"):
                j += 1
            spec = " ".join(l.lstrip("> ").strip() for l in lines[i:j])
            am = re.search(r"Alt text:\s*(.+?)(?:\s*$)", spec)
            alt = re.sub(r"\s+", " ", am.group(1)).strip() if am else f"Figure {fid} — {title}"
            alt = alt.replace('"', "&quot;")
            if am:
                with_alt += 1
            svgf = book_dir / "build" / "figures" / f"fig_{fid.replace('.', '_')}.svg"
            if svgf.exists():
                out.append(f'<figure><img src="{svgf.as_posix()}" alt="{alt}"/>'
                           f"<figcaption>Fig {fid} — {title}</figcaption></figure>\n")
                injected += 1
        out.append(lines[i])
        i += 1
    print(f"figures injected: {injected} ({with_alt} with alt text from the specification)")
    return "\n".join(out)
...
def build(book: str, out: pathlib.Path) -> None:
    ...
    # Full-bleed cover, generated by make_cover.py. Absent -> the book simply starts at the title
    # page, which is what happened before this existed; it is not a build failure.
    cover_file = book_dir / "build" / "cover.svg"
    cover_html = ""
    if cover_file.exists():
        cover_html = (f'<div class="coverpage"><img src="{cover_file.name}" alt="'
                      f'{cfg["run_title"]} — cover"/></div>')
        print("cover: included")
    else:
        print("cover: NONE — the volume will start at its title page")

    front = FRONT.format(title=cfg["title"], subtitle=cfg["subtitle"], domains=len(files))
    doc_html = ("<!doctype html><html lang='en-GB'><head><meta charset='utf-8'>"
                f"<style>html {{ string-set: booktitle \"{cfg['run_title']}\"; }}</style>"
                f"</head><body>{cover_html}{front}{''.join(toc)}{html}{app_html}{std_html}{cap_html}{gloss_html}"
                f"{''.join(ix)}</body></html>")
    ...
    # PDF/UA-1 produces a TAGGED pdf with a structure tree, which the conformance matrix claims as a
    # correction over the previous book and which was not in fact being produced. Fall back rather
    # than fail: an untagged book is better than no book, and the fallback says so out loud.
    try:
        doc.write_pdf(str(out), pdf_variant="pdf/ua-1")
        tagged = "tagged (PDF/UA-1)"
    except Exception as exc:                       # noqa: BLE001 - report and degrade honestly
        doc.write_pdf(str(out))
        tagged = f"NOT tagged — pdf/ua-1 failed: {type(exc).__name__}: {exc}"
    print(f"accessibility: {tagged}")
