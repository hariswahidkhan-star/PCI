...
The mapping is recorded in [`../laws/STANDARDS_CONCORDANCE.md`](../laws/STANDARDS_CONCORDANCE.md) section 3.
...
`docs/bok/00-style-spine.md` section 3, and the PCI Standard files — and then counting actual usage across the
...
registry rule quietly breached — that is a defect, and it is listed in **section 4 Issues found** with a
file, a line and a proposed resolution.

**Three tiers of severity are used in section 4:**
...
book build; the symbol registry in `FORMULAS.md`; and cross-book agreement.** Every issue in section 4 is
...
| **EAC** | PCL, PML, PFL | PCL Appendix B (line 104): "**`EAC` / `ETC`** — Estimate at completion / to complete; `EAC = AC + ETC`." Registered in `FORMULAS.md` section 1 in this sense only | PML KA 7.3: "Estimate at / to complete; `EAC = AC + ETC`" — identical. PFL KA 8.4 uses the same sense (`CTC` = `EAC − AC`). **PFL KA 4.2.3 (line 671) defines "`EAV` / EAC" as "NPV (cost PV) converted to a level annual equivalent"**, with the formula `EAC = PV / AF(r, n)` at lines 590–593 | **Yes — the sharpest inconsistency in the corpus.** One symbol, two different formulas, inside one book | **`EAC` = Estimate at Completion, suite-wide, without exception.** The annuity sense is already registered as **`EAV`** in `FORMULAS.md` section 3 and must use it. | **Genuine professional collision** — *equivalent annual cost* is standard finance usage and *estimate at completion* is standard controls usage. But unlike *sponsor* or *PV*, this one is **undeclared and unregistered**, and it collides with an inherited symbol. See section 4 Issue 1. |
| **PV** | PCL, PML, PFL | `FORMULAS.md` section 1: **`PV` (BCWS) = Planned Value (EVM context)**; separately **`PV(x)` = present value of amount `x`** | `FORMULAS.md` lines 26–28 carry an explicit **notation clash rule**: "`PV` = Planned Value in EVM contexts; discounting always writes 'present value' in words or `PV(x)`. PFL-AI … use `PV(x)`/`FV(x)` forms throughout and reserve bare `PV` for EVM material only." | **Yes — the rule is breached in both books.** PFL uses bare `PV` for present value **93 times across 12 domains** (D3: 32, D4: 27). PML uses bare `PV` for present value **33 times in D2** while using bare `PV` for Planned Value **80 times in D4/D7/D15** | **No single definition — context flag required**, exactly as `FORMULAS.md` already says. The *rule* is right; compliance is the problem. | **Genuine collision, correctly anticipated and then not enforced.** *EVM sense:* Planned Value (BCWS). *Discounting sense:* present value. See section 4 Issue 2. |
...
| **bankability** | PFL | PFL KA 5.3: "The degree to which contracts, risks and cash flows support limited-recourse financing on acceptable terms." | PFL KA 1.2 (*bankability triangle*) | None. Matches `TERMINOLOGY.md` section 2 word for word | As defined. | None. |
| **SPV** | PFL | PFL KA 1.1: "Ring-fenced single-purpose project company; the borrower and contract hub." | PFL KA 9.2 (*SPV vs HoldCo mezzanine*) | **Minor wording drift**: `TERMINOLOGY.md` section 2 says "The ring-fenced legal entity created to own, finance and operate a project"; the book says "single-purpose project company; the borrower and contract hub". Same concept | **The ring-fenced, single-purpose legal entity created to own, finance and operate a project; the borrower and the hub of the contract structure** (merges both). | None. |
| **DSCR** | PFL | PFL KA 10.2: "`CFADS` ÷ debt service; the period test lenders covenant on." | PFL KA 10.2 (*DSCR ÷ LLCR*) | None. Matches `FORMULAS.md` section 3 | As defined. | None. |
...
definitional work in the corpus and should be the model for the gaps in section 4.
...
The eight *legitimate* collisions catalogued in section 2 are **not** listed here — they need context flags,
...
`FORMULAS.md` section 1 registers `EAC` **only** as Estimate at Completion (inherited from PCL-AI D6) and
section 3 already provides **`EAV` — equivalent annual value** for precisely the D4 quantity. D4's key-terms
...
Completion suite-wide. Then add a line to `FORMULAS.md` section 1 under the notation-clash rule:
...
- `docs/books/registries/TERMINOLOGY.md` **section 1 line 9**, which lists *Baseline* among terms inherited
  **unchanged**
...
Then record *benefits baseline* in `TERMINOLOGY.md` section 2 as a PML-proposed term.

### Issue 4 — `control account` is narrowed against its inherited definition *(drift)*

**Files:**
- `docs/bok/00-style-spine.md` **line 64** (binding, inherited unchanged per `TERMINOLOGY.md` section 1)
...
**Files:**
- `docs/books/laws/PCI_STANDARDS_DRAFTING_MANUAL.md` **section 10** and `docs/books/laws/PCI_STANDARDS_CHARTER.md`
  **section 11** — *Governing principle*, both carrying the approved formulation verbatim. (The rule was first
  stated in the superseded `SUPERSEDED_LAW_SYSTEM_v0.md` section 8: "One approved formulation, everywhere, in
...
Standard files 22 / 0. `TERMINOLOGY.md` section 1 requires "British English throughout", and the editorial charter is
suite-wide.
...
**Files:** `docs/books/registries/TERMINOLOGY.md` section 2, which flags **only** *sponsor*.

Section 2 of this audit identifies eight terms with legitimately different professional meanings:
...
**Proposed resolution.** Add a **"section 4 Declared collisions"** section to `TERMINOLOGY.md` holding all
...
see section 2.1), and neither form reaches `TERMINOLOGY.md`; and `TERMINOLOGY.md` section 2 records **none** of
*project*, *programme*, *portfolio*, *governance*, *accountability* or *responsibility*, though it
governs terminology for all three books.

**Proposed resolution.** Add base entries using the Proposed suite definition column of section 2, placing
...
`TERMINOLOGY.md` section 2, which is where the suite's shared vocabulary is supposed to live.
