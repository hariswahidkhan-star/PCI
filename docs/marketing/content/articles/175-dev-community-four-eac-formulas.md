...
Three measurements, all taken to the same cut-off, produce everything else.
...
It can assemble the inputs, flag control accounts whose behaviour has changed, and test whether a claimed recovery has ever been achieved on comparable work. It should not emit a number you cannot explain. If you cannot say which method was used and which assumption it rests on, the forecast will not survive an audit or a board.
...
*Internal links: this how-to should link to [the earned value management pillar](https://projectcontrolsinstitute.org/earned-value-management) with that anchor, to [a worked month-end that produces these inputs](https://projectcontrolsinstitute.org/earned-value-worked-example) with that anchor, and to [how the estimate at completion reaches the accounts](https://projectcontrolsinstitute.org/eac-accounting) with the anchor "where forecasting meets financial reporting".*
