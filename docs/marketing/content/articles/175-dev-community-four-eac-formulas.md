...
The four EAC formulas are remaining work at budget, remaining work at the current CPI, remaining work at CPI × SPI, and a bottom-up re-estimate. They take the same earned value inputs and return four different final costs, because each assumes something different about the work still to come. Choosing between them is a judgement about cause, not a preference about arithmetic. Here is all four run on one dataset, with the code.
...
| Method | Formula | Result | VAC |
|---|---|---:|---:|
| 1. Remaining work at budget | AC + (BAC − EV) | **€26.2m** | −€2.2m |
| 2. Remaining work at current CPI | BAC ÷ CPI | **€29.2m** | −€5.2m |
| 3. Remaining work at CPI and SPI | AC + (BAC − EV) ÷ (CPI × SPI) | **€31.3m** | −€7.3m |
| 4. Bottom-up re-estimate | AC + a fresh ETC of €15.9m | **€28.3m** | −€4.3m |
...
### Where each method fails, and what it commits you to

| Method | The assumption you are signing | Fails when |
|---|---|---|
| Remaining work at budget | The loss is behind you and the rest runs at plan | The cause is systemic, which it usually is; the optimistic default, and the one auditors challenge first |
| BAC ÷ CPI | Performance to date continues to the end | CPI comes from an early or unrepresentative sample, or the remaining work has a different mix |
| CPI × SPI | Recovering the date will keep costing money | Schedule slip costs nothing to recover, such as waiting on a permit or an approval |
| Bottom-up | The team can re-estimate the remaining scope honestly | It takes real time, and a rushed one is the old estimate with a new date on it |
