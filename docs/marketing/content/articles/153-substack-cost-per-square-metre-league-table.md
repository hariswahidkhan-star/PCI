...
It can propose candidates, which is the tedious half, but only if somebody scores the proposals. On a validation run where a model proposed 40 projects and a cost manager confirmed 24 of the 30 genuine comparables in the set, precision is 24 ÷ 40 = 0.60 and recall 24 ÷ 30 = 0.80. Precision matters more here, because a false comparable enters the rate and gets signed. That measured-or-not distinction is what [where a model helps an estimator and where it does not](https://pciai.org/ai-for-cost-estimating-in-construction) turns on.
