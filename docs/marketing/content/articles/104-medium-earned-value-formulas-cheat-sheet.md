...
platform:      Medium
type:          guide
title:         Earned value formulas cheat sheet, properly explained
meta:          An earned value formulas cheat sheet with units, sign conventions and one dataset carried through every formula, plus earned schedule, TCPI and period CPI.
...
# Earned value formulas cheat sheet, properly explained

An earned value formulas cheat sheet needs four things a printed list normally leaves out: the units each measure is in, the sign convention, what the formula assumes, and one dataset carried through all of them so the numbers agree. This page gives all four, then shows where each formula stops working.

Every measure below is built from four inputs: PV, EV, AC and BAC.

## The four inputs, defined so each stands alone
...
**Actual cost (AC)** is the cost incurred against that same work to that same date, including accruals for goods received and work done but not yet invoiced.

**Budget at completion (BAC)** is the total authorised budget for the scope being measured, at whatever level you report: control account, work package or project.
...
| Measure | Formula | Units | These numbers | Reads as |
|---|---|---|---|---|
...
Sign convention throughout: negative is bad on the variances, below 1.00 is bad on the indices. TCPI is the exception, where a number above your current CPI is the warning.
...
The third line is worth doing by hand, because it is the one people get wrong. CPI × SPI = 0.853 × 0.906 = 0.773. Then 5.1 ÷ 0.773 = 6.60, and 3.4 + 6.60 = $10.00m.
...
That is not a coincidence and it is not a result. A forecast built by assuming today's efficiency continues will always report that today's efficiency is enough to hit it. Test the forecast against BAC, where TCPI is 1.109 against an actual 0.853, a 30% improvement required.

## Cumulative and period: one formula, two answers

Every index here can be run on cumulative figures or on the period alone, and the two say different things.

Say last month's cumulative position was EV $2.62m and AC $2.95m. The period figures are then EV of 2.9 − 2.62 = **$0.28m** and AC of 3.4 − 2.95 = **$0.45m**.

Period CPI = 0.28 ÷ 0.45 = **0.622**, against a cumulative CPI of 0.853.

Cumulative CPI is a heavy flywheel. It moves slowly, which makes it a fair basis for a forecast and a poor basis for noticing that this month collapsed. Report both, forecast from the cumulative, act on the period.

## Earned schedule: the formulas printed sheets omit
...
Earned schedule (ES) is the point on the baseline curve at which PV equals today's EV. Our EV is $2.9m; the baseline reaches $2.74m at month eight and $3.20m at month nine.

ES = 8 + (2.90 − 2.74) ÷ (3.20 − 2.74) = 8 + 0.16 ÷ 0.46 = **8.35 months**.

With actual time (AT) of 9.00 months: SV(t) = ES − AT = **−0.65 months**, and SPI(t) = 8.35 ÷ 9.00 = **0.928**.
...
| EAC = BAC ÷ CPI | Early in the job, when CPI rests on a small and unrepresentative sample |
| CPI × SPI method | The schedule slip has a cause that consumes no cost, such as a permit wait |
| TCPI | Measured against an EAC derived from CPI, where it simply hands back CPI |
...
Per cent complete on this sheet is 36.3%, built in baseline money to control a project. It is not a revenue figure. An input method of measuring progress towards a performance obligation under the applicable accounting standard uses costs incurred against total expected costs, which is different arithmetic serving a different purpose.

The two are related and they are not interchangeable. Where a controls percentage is handed to finance without that being understood, the delivery report and the ledger begin describing different projects.

Keeping both sides straight is the point of the [PCI AI Project Controls Leader (PCL-AI) credential](https://projectcontrolsinstitute.org/pcl-ai-certification), which examines project accounting alongside delivery across 13 domains and 61 knowledge areas.

## Frequently asked questions

**Do I need to memorise these for an exam?**
You need to be able to derive them, which is a different skill. Scenario-based examinations tend to hand you a dataset with a defect in it, an inconsistent cut-off or materials sitting inside actual cost, and the marks are in noticing that before you calculate anything. The formulas take an afternoon; the judgement takes practice.

**What is the difference between ETC and the remaining budget?**
Remaining budget is BAC − EV, the budgeted cost of work not yet earned. ETC is what that work will actually cost, which is the same figure only if you expect performance to return to plan immediately. Here the remaining budget is $5.1m and the CPI-based ETC is $5.98m; the gap between them is the forecast overrun.
...
Yes, and it is a familiar pattern. Under-claimed progress, generous earning rules or actuals that have not caught up will each lift CPI. So will a project quietly deferring work rather than doing it, since neither EV nor AC accrues on work nobody has started.

**What is a sensible reporting threshold on these variances?**
A percentage gate alone floods you with small accounts and misses large ones, so mature systems pair a percentage with an absolute value and add a second, higher absolute trigger that fires whatever the percentage. Thresholds belong in the baseline documentation, set once, and not adjusted at month-end to avoid writing a report.

---

*First published on projectcontrolsinstitute.org; the canonical points there. Medium links are nofollow, so this version is for readers rather than for link equity.*
