...
type:          pillar
title:         What are capital projects? The complete beginner's guide
meta:          What are capital projects? Capex versus opex, the stage-gate lifecycle, front-end loading, and the NPV arithmetic that decides whether one gets funded.
...
A capital project builds something that will still exist, and still earn, years after the project team has gone. A new terminal, a substation, a process train, a fleet replacement, a hospital wing, a data centre.

Three features separate them from ordinary work. They consume capital rather than operating budget, they are approved against a business case rather than against an annual plan, and they are governed by stage gates rather than by a line manager.

The consequence for anyone working on one is that the project is being judged on two clocks at once. There is the delivery clock, measured in progress and float, and the investment clock, measured in when the asset starts earning.
...
| Examples | Building a plant, replacing a bridge deck, installing new lines | Routine maintenance, consumables, hire for a short campaign |

The boundary case is worth learning, because it is argued on nearly every project. Work that restores an asset to its original condition is maintenance and goes to the income statement. Work that extends its life or increases its capacity is enhancement and may be capitalised.

That decision changes reported profit without changing a single physical activity, which is why the classification is agreed early and documented rather than settled at year end.
...
| Definition (FEL-3) | Define the chosen option to the point where cost and schedule can be committed | Sanction estimate, control schedule, contracting plan | Final investment decision |
...
Front-end loading is the deliberate investment of time and money in defining a project before committing to build it. It is the single largest lever available to a capital project, and it is available only before sanction.
...
A project sanctioned on thin definition does not become better defined by being started. It simply moves its unresolved questions into the execution phase, where they arrive as variations, standing time and rework.

The practical test is not how many documents exist at the gate, but whether the scope is stable enough that the estimate basis will still be true in six months. If a discipline lead cannot say what they are building, the estimate for it is a placeholder.

## How a capital project gets funded: the arithmetic

Investment committees compare projects against each other and against doing nothing. Three measures do most of that work, and a project controls professional should be able to derive all three.

Take a project costing £50m, spent at the start, generating £9m of net cash each year for ten years, with a required return of 8%.

**Net present value.** The present value of an even ten-year stream at 8% uses an annuity factor of (1 − 1.08⁻¹⁰) / 0.08 = 6.710. Multiply: £9m × 6.710 = **£60.39m**. Subtract the capital: £60.39m − £50m = **NPV of £10.39m**.

A positive NPV means the project earns more than the 8% the money is expected to earn elsewhere. That is the test it must pass.

**Payback.** Simple payback is £50m / £9m = **5.6 years**. It ignores the time value of money entirely, which is why it is a screening measure and never a decision measure.
...
The project clears its 8% hurdle by roughly 4.4 percentage points. That margin is what the delivery team is being asked to protect.
...
Push every cash inflow in that example back by one year, with no change to capital cost. The present value of the inflows becomes £60.39m / 1.08 = **£55.92m**, so the NPV falls to £5.92m.

A twelve-month delay has removed **£4.47m of value, which is 43% of the entire business case**, without a single pound of cost overrun.

That is the number to put in front of anyone arguing that an eight-week float loss is a scheduling matter. Time on a capital project is the business case, and float is being spent whether or not anyone is tracking it.
...
## The controls a capital project needs
...
**A work breakdown structure** that decomposes scope to the level at which someone can be held responsible. **A cost breakdown structure** that classifies spend so that a labour problem is visible as a labour problem.
...
At some point the money spent stops being a project cost and becomes an asset on the balance sheet. That transition is governed by accounting standards, and project controls supplies the evidence for it.
...
The practical implication is that the cost report has to be codeable to that boundary from the start. Trying to split three years of accumulated cost into capital and revenue at handover is a painful exercise with a predictable outcome.
...
Operations readiness is the role most often under-resourced. An asset that is mechanically complete and operationally unready has not started earning, and the business case only counts cash.

## Where capital projects go wrong

They go wrong at sanction more often than during execution. A project sanctioned on an immature estimate with a compressed schedule has already decided its outcome, and the delivery team spends three years managing a decision made in three months.

They go wrong when contingency is treated as a target rather than a provision. Contingency consumed at twice the rate of progress in the first year is a forecast, and it is usually a forecast nobody reads until month eighteen.
...
And they go wrong quietly, through optimism in the forecast, until the correction arrives in one large step. Small honest movements are always cheaper than one large one, both commercially and reputationally.

## Where capital projects sit in the PCI curriculum
...
The reason the curriculum carries both sides is that a capital project is simultaneously an engineering object and a balance sheet object. The people who can only see one of those are the reason the two versions of the truth diverge.
...
**What is FEL and how does it relate to stage gates?**
...
Not always, but the larger and more regulated they are, the more likely it is required. Even where a full earned value management system is not mandated, the underlying discipline of a time-phased baseline, rules of credit and objective progress measurement is what makes a capital project forecast defensible.
