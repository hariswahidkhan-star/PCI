...
type:          process-guide
title:         Month-end close for projects: the controls handshake
meta:          A working month-end close for projects: the day-by-day calendar, the accruals that decide CPI, five reconciliations that must tie, and the finance handover.
...
# Month-end close for projects: the controls handshake
...
Everything below assumes a calendar month and a five working-day close. Compress or extend the days; do not reorder the moves.
...
Pick the cut-off, write it in the procedure, and apply it to progress, commitments, accruals and the forecast alike. Where a survey genuinely cannot happen on the cut-off date, accrue the gap rather than shifting the date.
...
| WD+2 | Earned value computed; variances explained | Cost engineer | Cost report, variance narratives |
...
The two days that get squeezed are WD−3 and WD+3. Squeezing the survey produces unverified progress, and squeezing the forecast lock produces a number changed after the ledger closed. Both are the same failure with different names.
...
Goods receipted and not invoiced, **£0.510m**: the steel is on site and consumed, and the invoice has not arrived. Add it. Subcontract work performed and not yet certified, **£0.365m**: the labour was expended in the period. Add it. Prepaid site insurance of **£0.075m** relating to next period: reverse it.
...
Twelve points of CPI, produced by accrual discipline alone, and the error runs in the flattering direction by default because unbilled work is always the largest of the three adjustments.
...
Earn it and earned value becomes £7.390m, giving 7.390 / 7.280 = **1.015**, a package apparently returning better than budget on the strength of a delivery note. The same principle applies in the accounts, where cost incurred that is not proportionate to progress is excluded from the progress measure and the material is recognised at cost with no margin until it is installed.

The controls rule and the accounting rule agree here, which is unusual and worth using. If the earning rule and the revenue policy treat uninstalled materials the same way, one argument disappears from every close.
...
| Contract position | Cumulative revenue less billings and contract asset or liability | Nil | Applications issued outside the billing cycle |

Every one of these is a nil-tolerance tie, and that is deliberate. A tolerance on a reconciliation becomes a hiding place within two months.

Where a difference cannot be resolved in the close, book it, name it, and put it on the next month's action list with an owner. An unexplained difference carried silently is how a £4m surprise arrives in month eleven.

## Segregation: who claims and who verifies

The person who claims progress must not be the person who verifies it. On most sites the supervisor claims and the quantity surveyor verifies, and the cost engineer accepts only verified quantities into earned value.
...
Level of effort deserves the same scrutiny once a year rather than every month. Ask what share of the baseline sits on it and whether any discrete work has quietly migrated there, because level of effort earns with time and therefore reports a cost performance index near 1.00 whatever happens on site.

## What to hand finance at WD+4

A package, not a spreadsheet emailed at midnight. It contains actual cost by control account with the accrual schedule and its evidence, earned value with the signed progress claims behind it, and the forecast with the change log for every movement in the period.
...
The forecast is the item that carries reporting consequences. Estimate at completion is the denominator of the progress fraction under a cost-to-cost measure, so moving it moves recognised revenue and margin in the same period, as set out in [how the estimate at completion reaches the accounts](https://projectcontrolsinstitute.org/eac-accounting).
...
A financial controller is examined on cut-off, accruals and provisions but rarely on progress measurement. A cost engineer is examined on progress measurement but rarely on cut-off. The close is the fortnight where both are required at once and neither profession's examination covers the other side.

The PCI AI Project Finance Leader (PFL-AI) credential examines both, across 16 domains and 61 knowledge areas, with the Body of Knowledge weighted 40% finance and reporting, 40% project management and 20% governed AI. The PCI Standards behind it carry 113 mandatory standards and 532 process requirements, which is where the close discipline above is written down.

## Frequently asked questions

**How long should a project month-end close take?**
Five working days is achievable on a single contract with clean commitment data, and seven to eight is normal in a multi-project business consolidating several ledgers. The constraint is rarely computation. It is waiting for verified progress and for subcontractor valuations, so the improvements that work are moving the survey earlier and accruing rather than chasing.

**Should the cost report or the ledger be the source of truth?**
The ledger is the source of truth for cost incurred, and the cost report is the source of truth for what that cost bought. They are reconciled, not ranked. When the cost report shows a figure the ledger cannot support, the difference is an accrual, a coding error or a timing issue, and naming which one it is takes minutes if the reconciliation runs monthly.

**What evidence supports an accrual?**
A goods receipt note, a signed timesheet, a subcontractor's application, a delivery record, or a measured quantity with a rate. What does not support one is a percentage of last month. Accruals estimated as a proportion of the previous period are the mechanism by which a project drifts, because they always reproduce the previous month's error.

**Do small projects need this?**
The sequence, yes; the calendar, scaled. On a £2m project the survey, accruals and forecast can happen in one afternoon, and the five reconciliations still apply because their cost is minutes. What does not scale down is the segregation between claiming and verifying progress, which matters more on small teams, not less.

**How does the close connect to earned value reporting?**
The close produces the inputs; earned value interprets them. If cut-off, accruals and verified quantities are handled properly, the indices mean something, and if they are not, no amount of analysis rescues them. The method itself is set out on [the earned value management pillar](https://projectcontrolsinstitute.org/earned-value-management).

---

*Internal links: this guide should link to [how the estimate at completion reaches the accounts](https://projectcontrolsinstitute.org/eac-accounting) with that anchor, to [the earned value management pillar](https://projectcontrolsinstitute.org/earned-value-management) with that anchor, and to [IFRS for project controls](https://projectcontrolsinstitute.org/ifrs-for-project-controls) with that anchor; the EAC accounting and project cash flow pieces should link back here with the anchor "month-end close for projects".*
