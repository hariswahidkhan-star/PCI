---
platform:      LinkedIn post
type:          linkedin-post
title:         Four missing records, and where the finding really goes
meta:          Thirty progress claims sampled, four unevidenced, £0.80m extrapolated — and the chain that turns a records finding into a reported revenue finding.
primary_kw:    credential as an auditor
secondary_kw:  progress claims, earned value evidence, project assurance, PFL-AI
pillar:        Certification and careers
credential:    suite
target_domain: credentialfinder.org
canonical:     canonical -> credentialfinder.org/choosing-a-credential-as-auditor
schema:        Article
word_count:    n/a — 1,892 characters
hashtags:      #InternalAudit #ProjectControls #ProjectFinance #RiskManagement
ab_id:         none — comparison plan, batch 14
compares:      [audit credentials, accountancy and finance qualifications, the established project controls certifications]
---

# Four missing records, and where the finding really goes

*LinkedIn post. No link in the body — it goes in the first comment.*

---

You sample 30 progress claims on a capital project. Four have no measurement record.

4 ÷ 30 = 13.3%.

£6.0m of progress was claimed in the period, so 6.0 × 0.133 ≈ £0.80m claimed without evidence behind it.

Written up as "records were missing", that goes in the appendix and somebody promises a template.

Now write it the other way.

Unsupported progress inflates earned value. That raises the cost performance index, which lowers the forecast out-turn. And where progress is measured by costs incurred against total expected costs, a lower forecast raises the percentage complete — and the revenue recognised with it.

Same evidence. Now the finding is that £0.80m of unevidenced progress has flowed through an index into a reported number, and it lands in the executive summary.

Audit credentials examine governance, risk, control and the assurance process — a full profession, and it transfers to projects intact. What does not come with it is the technical subject those controls govern: earning rules, cut-off dates, accruals, the forecast at completion.

Four artefacts. That is the whole gap, and it is smaller than a second profession.

Where a project credential is the wrong answer: if you audit information systems, the one built for that subject is right; if your firm's progression names a certification, take it.

The narrow case is capital project work where the findings that matter are numbers. PFL-AI covers that boundary: 16 domains, 61 knowledge areas.

Disclosure: credentialfinder.org is operated by Project Controls Institute Global, which awards it.

PCI is new and not accredited — read the syllabus and judge it. It is published free, in full, before anyone pays.

#InternalAudit #ProjectControls #ProjectFinance #RiskManagement

---

*First comment: https://credentialfinder.org/choosing-a-credential-as-auditor — the auditor guide.*

*Internal-linking note: one link only, in the first comment, to the auditor guide on
credentialfinder.org — it answers the question the post raises about what sits in the gap between
control testing and testing the number. No body link, because LinkedIn suppresses reach on posts
carrying one, and no link or name for any other awarding organisation.*
