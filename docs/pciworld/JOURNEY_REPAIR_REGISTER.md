...
| OAuth groundwork | Dedicated-domain plan had no standard authorization path | P1 | Only the transitional fragment-handoff bridge existed | OAuth 2.1-shaped layer: client registry (exact redirect-URI matching, no prefixes/wildcards), `/api/oauth/world/authorize` (portal identity, impersonation refused) issuing single-use 2-min PKCE-bound codes (S256 only), `/api/oauth/world/token` with uniform `invalid_grant`, failed-guess code burning, replay detection that revokes the stolen code's minted session, retention-aware sweep. A future `pciworld.org` app is one registry row | `Endpoints/WorldOAuth.cs`, `Data/WorldSchema.cs`, `Core/WorldRotation.cs` | `WorldOAuthTests` (5) | it19 |
| OAuth registry admin | Registering a client required a deploy | P1 | Registry seeded in schema only | Owner-gated CRUD over the client registry (`/api/world-admin/oauth-clients*`, audited) with hard validation (relative path or https URL only, no wildcards/fragments/whitespace, ≤5 entries, exact-match semantics preserved); seeded first-party client editable but never deletable (load-bearing); deleting a client kills its outstanding codes; Users-tab card UI. MySQL-parity fix folded in (no LIMIT-in-IN, no self-referencing update subqueries) — first full MariaDB validation of the World suites (CI-identical invocation, green) | `Endpoints/WorldOAuth.cs`, `Endpoints/WorldAdmin.cs` | `WorldOAuthTests` (6) | it20 |
| OAuth consent contract | Third-party-shaped clients could sign a person in with no consent step | P1 | Registry had no first-party notion | Server-enforced consent: clients not flagged `first_party` (new column, settable only via the owner endpoints; seeded `pciworld-app`=1) require explicit `consent:true` at authorize or get `consent_required` with the client name + the fixed scope description ("sign you in to PCI World practice — never certifications, exams, or billing") for the caller to render a consent screen; nothing issues until the person decides. First-party stays frictionless; flag flips take effect immediately, both ways. Admin card gains the first-party checkbox. Consent-screen UI in the portal is a follow-up (the contract is server-side, so no UI can bypass it) | `Endpoints/WorldOAuth.cs`, `Endpoints/WorldAdmin.cs`, `Data/WorldSchema.cs` | `WorldOAuthTests` (7) | it21 |
| PW-US-058 (scope guard) | Email-change divergence risk between products | P1 | World displayed nothing about where the sign-in email lives | The dashboard aggregate exposes the canonical email; the account page shows "Signed in as … — managed by your PCI account, change it in the student portal settings" (linked accounts get the portal link; the change propagates automatically because the identity IS shared) — World never grows its own email editor. Support diagnostics gain first_entered_at/onboarded_at journey anchors | `Endpoints/WorldAccount.cs`, `Core/WorldPages.cs`, `Core/WorldIdentity.cs` | diagnostics fact extended | it22 |
| P2 (pagination + confirmation) | Audit/report history truncated at 200; destructive admin actions fired on one click | P2 | Hard LIMIT with no total; no confirm/pending states | `/api/world-admin/audit` and `/reports` gain limit/offset/total; the shell pages both with Older buttons (append + rebind, totals in the heading); challenge Retire confirms with the challenge code and explains scope/reversibility; lifecycle and resolve buttons disable while their call runs | `Endpoints/WorldAdmin.cs` | live-boot JS-parse gate; endpoints are thin SQL over tested pagination patterns | it18 |
...
2. ~~A real OAuth 2.1 authorization layer~~ — LANDED (it19–21): authorization-code + PKCE (S256
   ...
   consent-screen UI (the contract is server-side, so no UI can bypass it meanwhile) and, for a
   true separate-origin deployment, id-token/OIDC claims plus per-client CORS entries.
