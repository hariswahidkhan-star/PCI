...
| 2 — Foundation launch | 30 reviewed challenges, archive search/filter, Decision Replay UI, Coach (behind the AI gate below), content correction flow, full E2E/a11y/security passes | **Partially delivered**: 30-challenge validated library across all 18 engine families / 5 tracks / 20+ industries; archive industry/difficulty/track filters; password reset (no-enumeration, single-use, sign-out-everywhere). Remaining: Coach (gate below). Content-correction flow **delivered** (public report form → admin Reports queue with resolve-note audit). First Playwright pass **delivered** (`frontend/e2e/portal-world.spec.ts`: anonymous journey incl. public-page answer-leak assertions, archive filters, report flow, axe WCAG 2.x AA scan of the home page, separate-admin login, PCI-admin-carries-no-World-nav) |
